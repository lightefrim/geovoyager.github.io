...
// Update click handlers to include sound
...
// Add event listener for next button to load next city
document.getElementById('nextBtn').addEventListener('click', () => {
    const nextBtn = document.getElementById('nextBtn');
    // Hide the Next button and load the next city
    nextBtn.style.display = 'none';
    nextCity();
});
...
function startTimer() {
    ...
    timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            clearInterval(timer);
            alert(`Time's up! That was ${currentCity.name}.`);
            nextCity();
        }
    }, 1000);
}
